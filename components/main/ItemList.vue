...
<template>
  <div class="list">
    <div>
      <!-- 종목정보 -->
      <div v-if="bullet" class="bullet">Today</div>
      <div class="obj">{{ item }}</div>
    ...
    <div>
      ...
      <div class="signal-info">
        <div v-if="infoTxt" class="txt">{{ signalInfo }}</div>
        <!-- signalInfo = 보유 45일째  -->
...
export default {
  ...
  methods: {
    getSignalText() {
      return this.signalClass === 'sig01' ? '매수' : this.signalClass === 'sig02' ? '매도' : this.signalClass === '' ? '-' : this.signalText
    }
  }
}
</script>
